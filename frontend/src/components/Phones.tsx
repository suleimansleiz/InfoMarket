...
const Phones: React.FC = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const response = await api.get("/api/infomarket/v1/items/category/Phones", {});
  ...
  const handleFilterChange = async (type: string) => {
    ...
    try {
      ...
      if (type === "All") {
        response = await api.get("/api/infomarket/v1/items/category/Phones");
      } else if (type === "Recents") {
        response = await api.get("/api/infomarket/v1/items/category/Phones/recent");
      } else {
  ...
  return (
    <div className="pages-container">
      ...
      <h1 className="welcome-header text-center">Phones</h1>
